...
const ServiceDetails = () => {
  ...
  useEffect(() => {
    const fetchService = async () => {
      try {
        const res = await axios.get(`http://localhost:5000/api/services/single/${id}`);
        setService(res.data);
      } catch (err) {
      ...
      <div className="row">
        ...
        <div className="col-md-8">
          ...
          <div
            ...
            onClick={() => navigate(`/vendors/${service.vendor._id}`)}
          >
            <img
            ...
            />
            <h6 className="mb-0">
              {service.vendor.fullname || service.vendor.companyName}
            </h6>
          </div>
        </div>
      </div>
